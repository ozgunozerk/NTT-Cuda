...
__global__ void GSBasedINTTInner(unsigned long long a[], unsigned long long psiinv_powers[], int length)
{
    register int global_tid = blockIdx.x * nttBlockSize + threadIdx.x;
    register int step = __fdividef(__fdividef(N, length), 2);
    register int psi_step = __fdividef(global_tid, step);
...
__global__ void GSBasedINTTMerged(unsigned long long a[], unsigned long long psiinv[])
{
    ...
    {
        GSBasedINTTInner <<< __fdividef(__fdividef(N, nttBlockSize), 2), nttBlockSize >>> (a, psiinv, m);
    }
}
