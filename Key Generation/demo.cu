...
int main()
{
    ...
    /*vector<unsigned long long> q = { 274877562881, 274877202433, 274877153281 };
    ...
    vector<unsigned long long> q = { 8796092858369, 8796092792833, 17592186028033, 17592185438209 };
    ...
    for (int i = 0; i < q_amount; i++)
    {
        q_bit_lengths.push_back(log2(q[i]) + 1);
    ...
    {
        inv_q_last_mod_q.push_back(modinv128(q[q_amount - 1] % q[i], q[i]));
    ...
    for (int i = 0; i < q_amount; i++)
        psiinv_roots.push_back(modinv128(psi_roots[i], q[i]));
    ...
    for (int i = 0; i < q_amount - 1; i++)
    {
        mult_t = (host64x2(mult_t, q[i]) % t).low;
        mult_g = (host64x2(mult_g, q[i]) % gamma).low;
    }
    ...
    for (int i = 0; i < q_amount; i++)
    {
        prod_t_gamma_mod_q.push_back((prod_t_gamma % q[i]).low);
    }
    ...
    for (int i = 0; i < q_amount; i++)
    {
        ...
        inv_punctured_q.push_back(modinv128(temp.low, q[i]));
    }
    ...
    encryption_rns(c, public_key, in, u, e, n, streams, q, q_bit_lengths, mu_array,
    ...
    decryption_rns(c, secret_key, q, q_bit_lengths, mu_array, psi_table_device, psiinv_table_device,
