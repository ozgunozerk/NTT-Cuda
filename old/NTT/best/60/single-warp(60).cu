...
__global__ void CTBasedNTTInner(unsigned long long global_array[], unsigned long long psi_powers[])
{
    ...
    register unsigned long long z = psi_powers[local_tid % 32];
    ...
    for (int length = 1; length < 17; length *= 2)  //
    {
        ...
        for (int iteration_num = 0; iteration_num < (N / nttBlockSize) / 2; iteration_num++)
        {
            ...
            register unsigned long long psi = __shfl_sync(0xffffffff, z, length + psi_step);

            register unsigned long long first_target_value = global_array[target_index];
            register uint128_t temp_storage = global_array[target_index + step];  // this is for eliminating the possibility of overflow
            ...
            global_array[target_index] = target_result;

            if (first_target_value < second_target_value)
                first_target_value += q_;

            global_array[target_index + step] = first_target_value - second_target_value;
        }
...
__global__ void GSBasedINTTInner(unsigned long long a[], unsigned long long psiinv_powers[])
{
    ...
    register unsigned long long z = psiinv_powers[local_tid % 32];
    ...
    for (int length = N / 2; length > 16; length /= 2))
    {
        ...
        for (int iteration_num = 0; iteration_num < (N / nttBlockSize) / 2; iteration_num++)
        {
            ...
            singleBarrett(temp_storage);
            a[target_index + step] = temp_storage.low;
        }
    ...
    for (int length = 16; length > 0; length /= 2)
    {
        ...
        for (int iteration_num = 0; iteration_num < (N / nttBlockSize) / 2; iteration_num++)
        {
            ...
            register unsigned long long psiinv = __shfl_sync(0xffffffff, z, length + psi_step);
...
__global__ void GSBasedINTTMerged(unsigned long long a[], unsigned long long psiinv[])
{
    ...
    {
        GSBasedINTTInner <<< (N / nttBlockSize) / 2, nttBlockSize >>> (a, psiinv);
    }
}
...
int main()
{
    ...
    printf("\nTime taken to perform forward NTTs:  %f ms \n", time);
    total_time += time;
