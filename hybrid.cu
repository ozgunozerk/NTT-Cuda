...
__global__ void CTBasedNTTInnerSingle(unsigned a[], unsigned psi_powers[], unsigned int l)
{
    ...
    for (int length = l; length < N; length *= 2)
    {
        ...
        for (int iteration_num = 0; iteration_num < (N / nttBlockSize / l) / 2; iteration_num++)
        {
            ...
            /*if (length == 8 && blockIdx.x == 4)
            {
                printf("block id = %d, global id = %d, psi pow = %d, target index = %d, 2nd target index = %d, iteration num = %d\n", blockIdx.x, local_tid, length + psi_step, target_index, target_index + step, iteration_num);
            }*/
