...
__global__ void CTBasedNTTInner(unsigned a[], unsigned psi_powers[])
{
    ...
    __shared__ unsigned shared_array[N];

    #pragma unroll
    for (int iteration_num = 0; iteration_num < (N / nttBlockSize); iteration_num++)
    {
...
{
    register int global_tid = threadIdx.x;
    ...
    for (int iteration_num = 0; iteration_num < (N / nttBlockSize); iteration_num++)
    {
        register int local_tid = global_tid + iteration_num * nttBlockSize;
        shared_array[local_tid] = a[local_tid];
    }
    ...
    for (int length = (N / 2); length > 0; length /= 2)
    {
        ...
        for (int iteration_num = 0; iteration_num < (N / nttBlockSize / 2); iteration_num++)
        {
            register int psi_step = (global_tid + iteration_num * nttBlockSize) / step;
            register int target_index;
            
            target_index = psi_step * step * 2 + (global_tid + iteration_num * nttBlockSize) % step;
            ...
            singleBarrett(temp_storage);
            shared_array[target_index + step] = temp_storage;
        }
    ...
    {
        register int local_tid = global_tid + iteration_num * nttBlockSize;
        a[local_tid] = shared_array[local_tid];
    }
    __syncthreads();
}
...
int main()
{
    ...
    CTBasedNTTInner << <N / nttBlockSize / 2, nttBlockSize, 0, ntt1 >> > (d_a, d_psi);
    CTBasedNTTInner << <N / nttBlockSize / 2, nttBlockSize, 0, ntt2 >> > (d_b, d_psi);
